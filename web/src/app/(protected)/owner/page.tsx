...
export default function OwnerProjectsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getOwnedProjects(page, 10, status, sortBy, order);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Pantau progres pembangunan dan lihat laporan harian dari tim di
              lapangan.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 mb-8">
          ...
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-wrap items-center gap-3 lg:w-2/3">
            ...
            <select
              ...
              className="bg-gray-50 border-none text-sm rounded-xl px-4 py-2 text-gray-700 outline-none focus:ring-2 focus:ring-amber-500/20 cursor-pointer"
            >
            ...
            <button
              ...
              className="px-4 py-2 bg-gray-50 rounded-xl text-sm font-medium text-gray-600 hover:bg-amber-50 hover:text-amber-600 transition-all border-none cursor-pointer"
            >
              {order === "asc" ? "A-Z ↓" : "Z-A ↓"}
            </button>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                {loading ? (
                  ...
                  projects.map((p) => (
                    <tr
                      ...
                      onClick={() =>
                        router.push(`/owner/project/${p.id}/documentation`)
                      }
                    >
                      <td className="p-5">
                      ...
                      <td className="p-5 text-right">
                        <button
                          className="inline-flex items-center justify-center p-2 text-amber-600 hover:bg-amber-100 rounded-lg bg-transparent border-none cursor-pointer transition-colors"
                          title="Lihat Laporan"
                        >
          ...
          <div className="p-5 bg-gray-50/50 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
            <p className="text-xs text-gray-500 font-medium tracking-wide uppercase">
              Halaman <span className="text-amber-600">{page}</span> dari{" "}
              {totalPages}
            </p>
