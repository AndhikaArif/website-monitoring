  ...
  return (
    <div className="min-h-screen bg-gray-50 text-black">
      <div className="p-4 sm:p-6 max-w-7xl mx-auto">
        ...
        {/* ================= HEADER SECTION ================= */}
        <div className="flex flex-col xl:flex-row xl:justify-between xl:items-center mb-8 gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 border-t-4 border-t-amber-500">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              {currentUser?.name
                ? `Halo Bapak/Ibu ${currentUser.name}, `
                : "Selamat datang. "}
              {projectStartDate
                ? `Pantau progres pengerjaan sejak ${projectStartDate.toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}.`
                : "Pantau progres proyek rumah Anda secara transparan."}
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full xl:w-auto">
            ...
            <div className="relative flex-1 sm:w-64">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:bg-white focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 text-black placeholder-slate-400 transition-all"
              />
              {searchInput && (
                <button
                  type="button"
                      ...
                      {dateGroup.sessions.PAGI.length > 0 ? (
                        <div className="space-y-3">
                          {dateGroup.sessions.PAGI.map((docItem) => (
                            <Link
                              key={docItem.id}
                              href={`/owner/project/${projectId}/documentation/${docItem.id}`}
                              className="block group"
                            >
                              <div className="flex gap-4 bg-slate-50/50 border border-slate-200 rounded-xl p-3 hover:border-amber-400 hover:bg-white hover:shadow-sm transition-all">
                                ...
                                <div className="flex flex-col justify-between min-w-0">
                                  <div>
                                    <h3 className="font-bold text-sm text-slate-800 line-clamp-1 group-hover:text-amber-600 transition-colors">
                                      {docItem.workArea}
                                    </h3>
