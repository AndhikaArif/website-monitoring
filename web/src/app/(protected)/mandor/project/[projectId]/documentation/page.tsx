  ...
  const fetchDocs = useCallback(async () => {
    try {
      ...
      const res = await getProjectDocumentations({
        ...
        limit,
        page,
        // Menggunakan debouncedSearch, bukan searchInput langsung
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end mb-8 gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 border-t-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1 mb-4 sm:mb-0 max-w-lg">
              Pantau progres proyek secara transparan melalui bukti lapangan
              yang diunggah oleh Kepala Tukang.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            ...
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="block w-full text-black pl-10 pr-3 py-2.5 border border-slate-200 rounded-xl leading-5 bg-slate-50 placeholder-slate-400 focus:outline-none focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 sm:text-sm transition-all"
              />
              {searchInput && (
                <button
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (rawDate) {
                    ...
                    // Memasukkan format tanggal ke dalam searchInput
                    setSearchInput(`${day}-${month}-${year}`);
                  } else {
                    setSearchInput("");
                  }
                }}
                className="block w-full text-black px-4 py-2.5 border border-slate-200 rounded-xl leading-5 bg-slate-50 focus:outline-none focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 sm:text-sm transition-all cursor-pointer"
              />
            </div>
          </div>
        </div>
        ...
        {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {docs.map((doc) => (
                <div
                  ...
                  className="bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-lg hover:border-purple-300 hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col group relative cursor-pointer"
                >
                  {/* IMAGE HEADER */}
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <h3
                        className="font-bold text-slate-800 text-lg leading-tight mb-2 line-clamp-1 group-hover:text-purple-700 transition-colors"
                        title={doc.workArea}
                      >
                        {doc.workArea}
                      </h3>
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 bg-purple-50/30 -mx-5 -mb-5 p-3 rounded-b-2xl">
          ...
          <div className="flex flex-col items-center justify-center py-20 px-4 bg-white rounded-3xl border-2 border-dashed border-slate-200">
            ...
            <p className="text-slate-500 text-center max-w-sm mb-0">
              {debouncedSearch
                ? `Tidak ada Laporan yang cocok dengan pencarian "${debouncedSearch}".`
                : "Kepala Tukang belum mengirimkan dokumentasi progres untuk proyek ini."}
            </p>
          </div>
        )}
      </div>
